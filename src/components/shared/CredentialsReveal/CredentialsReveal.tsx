...
export default function CredentialsReveal({ heading, message, password, actionLabel, onAction }: CredentialsRevealProps) {
  ...
  async function handleCopy() {
    try {
      ...
      // clipboard access denied — the password is still visible to copy manually
    }
  }
  ...
  return (
    <div className="flex flex-col items-center text-center gap-4 py-2">
      ...
      <div className="w-full flex items-center justify-between gap-2 rounded-lg border border-border bg-surface-alt px-3 py-[10px]">
        <code className="text-[14px] font-mono font-semibold text-primary tracking-wide">{password}</code>
        <button
          type="button"
          onClick={handleCopy}
