...
interface ConfirmArchiveManagerModalProps {
  managerName: string
  isArchiving: boolean
  onConfirm: () => void
  onCancel: () => void
...
export default function ConfirmArchiveManagerModal({
  managerName,
  isArchiving,
  onConfirm,
  onCancel,
}: ConfirmArchiveManagerModalProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={onCancel}>
      ...
      >
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <h1 className="text-[18px] font-semibold text-primary">Archive {managerName}?</h1>
          <p className="text-[13px] text-secondary leading-relaxed">
            They&apos;ll be signed out and lose access to every store they manage. Their history stays on file,
            and you can unarchive them from the Archived tab at any time.
          </p>
          <div className="flex w-full gap-3 mt-2">
            ...
            >
              {isArchiving ? 'Archiving…' : 'Archive manager'}
            </button>
          </div>
        </div>
      </div>
